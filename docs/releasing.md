...
工作流 runner 通过校园网联合镜像站（CERNET）执行 Nix 单用户安装，并禁用安装器默认添加的官方 channel，再从 CERNET 的 `nixpkgs-unstable` channel 安装 `nix-update`；Nix binary cache 使用 CERNET 的 `nix-channels/store`。项目 flake 的 nixpkgs inputs 是例外，仍固定使用 NJU Git 镜像；两个 inputs 分别跟踪 `nixos-unstable` 和 `nixpkgs-26.05-darwin`。

`nix-update --build` 的 Go 模块下载显式使用 CERNET 的 Go module proxy：`https://mirrors.cernet.edu.cn/go`。
