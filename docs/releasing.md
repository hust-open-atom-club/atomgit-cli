...
工作流 runner 通过清华 TUNA 镜像执行 Nix 单用户安装，并禁用安装器默认添加的官方 channel，再从 TUNA 的 `nixpkgs-unstable` channel 安装 `nix-update`；Nix binary cache 将 TUNA 显式设为更高优先级，官方 cache 作为回退。项目 flake 的 nixpkgs inputs 通过 CERNET Git 镜像进行浅克隆，分别跟踪 `nixos-unstable` 和 `nixpkgs-26.05-darwin`。
