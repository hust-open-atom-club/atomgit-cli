...
- **Arch Linux / AUR**：Arch Linux 或使用 AUR 的系统可选稳定源码版 `atomgit-cli`、稳定二进制版 `atomgit-cli-bin`（免编译）或开发版 `atomgit-cli-git`（跟随 main 分支最新提交）。AUR 是用户仓库，并非 Arch 官方仓库；三个包由项目维护者 `moyigeek` 维护并随 Release 更新，安装与使用按 AUR 规则自行承担风险。
