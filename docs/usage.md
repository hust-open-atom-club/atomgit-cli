...
# 将仓库转移到其他用户或组织命名空间（默认需要确认）
ag repo transfer owner/repo --to destination
...
`ag repo transfer` 会改变仓库所有者，并可能影响仓库 URL、访问权限和自动化配置。命令在转移前显示源仓库和目标命名空间并要求确认，`--yes` 只跳过确认；组织所属仓库仍需输入 AtomGit 账户密码。交互终端中密码会隐藏输入，非交互场景必须结合 `--yes --password-stdin` 从标准输入读取。POST 成功后命令会读取目标仓库，只有完整名称和浏览器 URL 得到确认才报告成功。该命令**不会修改本地 Git remote**，转移完成后请根据输出的新 URL 手动检查并更新本地 remote。
