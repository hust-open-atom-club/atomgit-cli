...
async function inspectArtifacts(directory, tag) {
  ...
  assertExactFiles(directories, ["npm"], "release directory");
...
function sourceArchiveNames(tag) {
  return [`${tag}.zip`, `${tag}.tar.gz`, `${tag}.tar.bz2`, `${tag}.tar`];
}

function validateSourceArchives(release, tag, requireComplete = false) {
  const expected = new Set(sourceArchiveNames(tag));
  const actual = new Set();
  for (const asset of release.assets || []) {
    if (asset.type !== "source") continue;
    if (!expected.has(asset.name)) throw new Error(`release contains unexpected source archive ${asset.name}`);
    if (actual.has(asset.name)) throw new Error(`release contains duplicate source archive ${asset.name}`);
    actual.add(asset.name);
  }
  if (requireComplete) assertExactFiles([...actual].sort(), [...expected].sort(), "source archive");
}

function attachmentMap(release, expectedNames, tag) {
  validateSourceArchives(release, tag);
...
async function publishRelease(plan, options = {}) {
  ...
  let assets = attachmentMap(release, expectedNames, options.version);
  for (let index = 0; index < plan.artifacts.length; index += 1) {
    ...
    if (uploaded.status !== 0) {
      release = await getRelease(runAg, options.repo, options.version, env);
      assets = release ? attachmentMap(release, expectedNames, options.version) : new Map();
      if (!assets.has(artifact.name)) {
    ...
    release = await getRelease(runAg, options.repo, options.version, env);
    assets = attachmentMap(release, expectedNames, options.version);
  }
  ...
  validateSourceArchives(release, options.version, true);
