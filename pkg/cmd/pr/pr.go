...
func newCmdPRList(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, pr := range prs {
				fmt.Printf("#%s %s [%s]\n", pr.GetNumber(), cmdutil.SanitizeTerminal(pr.Title), pr.State)
			}
...
func newCmdPRView(f *cmdutil.Factory) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Printf("Title: %s\n", cmdutil.SanitizeTerminal(pr.Title))
			fmt.Printf("State: %s\n", pr.State)
			fmt.Printf("Author: %s\n", cmdutil.SanitizeTerminal(pr.User.Login))
			fmt.Printf("URL: %s\n", cmdutil.SanitizeTerminal(pr.HTMLURL))
			fmt.Printf("Branch: %s -> %s\n", cmdutil.SanitizeTerminal(pr.Head.Ref), cmdutil.SanitizeTerminal(pr.Base.Ref))
			if len(labels) > 0 {
				labelNames := make([]string, len(labels))
				for i, label := range labels {
					labelNames[i] = cmdutil.SanitizeTerminal(label.Name)
				}
				fmt.Printf("Labels: %s\n", strings.Join(labelNames, ", "))
			}
			fmt.Printf("Created: %s\n", pr.CreatedAt)
			if pr.Body != "" {
				fmt.Printf("\n%s\n", cmdutil.SanitizeTerminal(pr.Body))
			}
...
func newCmdPREdit(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Printf("Updated PR #%s: %s\n", pr.GetNumber(), cmdutil.SanitizeTerminal(pr.HTMLURL))
