...
func TestIssueCommentEditPropagatesCurrentUserError(t *testing.T) {
	factory := &cmdutil.Factory{Config: issueCommentTestConfig{userErr: errors.New("user lookup failed")}}
...
func issueJSONResponse(status int, body string) *http.Response {
...
func TestIssueCommentEditVerifiesParentBeforePatch(t *testing.T) {
	var methods []string
	factory := &cmdutil.Factory{
		...
		HttpClient: func() (*http.Client, error) {
			return &http.Client{Transport: issueCommentRoundTripFunc(func(req *http.Request) (*http.Response, error) {
				...
				switch req.Method {
				case http.MethodGet:
					if req.URL.Path != "/api/v5/repos/alice/demo/issues/8/comments" {
						t.Fatalf("parent lookup path = %s", req.URL.Path)
					}
					return issueJSONResponse(http.StatusOK, `[{"id":42,"body":"old","user":{"login":"alice"}}]`), nil
				case http.MethodPatch:
					if req.URL.Path != "/api/v5/repos/alice/demo/issues/comments/42" {
...
func TestIssueCommentDeleteRejectsCommentFromAnotherIssue(t *testing.T) {
	deleteCount := 0
	factory := &cmdutil.Factory{
		Config: issueCommentTestConfig{},
		HttpClient: func() (*http.Client, error) {
	...
	err := cmd.RunE(cmd, []string{"alice/demo", "8", "42"})
	if err == nil || !strings.Contains(err.Error(), "does not belong to issue #8") {
